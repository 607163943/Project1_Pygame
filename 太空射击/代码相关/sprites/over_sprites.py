...
class Label(pg.sprite.Sprite):
    ...
    def __init__(self,window,width,height,color,x,y,font_size,font_color,text):
        self.window=window
        # 将标签实列添加到精灵组
        self.groups=(self.window.over_all_sprite,self.window.over_labels)
        super().__init__(self.groups)
        self.image=pg.surface.Surface((width,height))
        self.image.set_colorkey(OverConfig.BLACK)
        # 指定标签位置
    ...
    def draw_text(self,font_size,font_color,text):
        font=pg.font.Font(OverConfig.FONT_NAME,font_size)
...
class Button(pg.sprite.Sprite):
    ...
    def __init__(self,window,width,height,color,x,y,font_size,font_color,text):
        self.window=window
        # 将按钮添加到精灵组
        self.groups=(self.window.over_all_sprite,self.window.over_buttons)
        super().__init__(self.groups)
        # 绘制按钮图像
        self.image=pg.surface.Surface((width,height))
    ...
    def mouse(self):
        ...
        if self.rect.collidepoint(self.window.mouse_x,self.window.mouse_y):
            ...
            if self.window.is_mouse:
                self.window.over_playing=False
                self.window.is_mouse=False
            self.image.fill(OverConfig.WHITE)
            self.draw_text(*self.font_tuple)
        else:
            self.image.fill(OverConfig.GREY1)
            self.draw_text(*self.font_tuple)
        self.window.is_mouse=False